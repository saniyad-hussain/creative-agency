...
const Orders = () => {
	...
	useEffect(() => {
		fetch(`https://obscure-badlands-01285.herokuapp.com/serviceOrder/${id}`)
			.then((res) => res.json())
			.then((data) => {
	...
	const handleOrderSubmit = (e) => {
		...
		fetch('https://obscure-badlands-01285.herokuapp.com/postOrder', {
			method: 'POST',
			body: formData,
		})
				...
				<div className="order-form mx-5 col-md-6 p-5">
					<form action="" onSubmit={handleOrderSubmit}>
						<div className="form-group">
							<input onBlur={handleBlur} type="text" name="name" className="form-control" value={loggedInUser.name} required />
						</div>
						<div className="form-group">
							<input onBlur={handleBlur} type="email" name="email" className="form-control" value={loggedInUser.email} required />
						</div>

						<div className="form-group">
							<input type="text" name="project" className="form-control" value={userOrder} />
						</div>
